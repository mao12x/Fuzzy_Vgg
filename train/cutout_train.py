...
data = ImageDataBunch.from_folder(PATH, train="train",valid="train/",
                                  test="test",
                                  valid_pct=.4,
                                  ds_tfms=transform,
                                  size=112,bs=64,
...
print(data)
learn = cnn_learner(data, models.alexnet, metrics=[FBeta(average='weighted'),accuracy], wd=1e-1, callback_fns=ShowGraph)
learn.fit_one_cycle(8)
learn.model_dir = Model_Path
learn.save('pretrain_alexnet_1')

#stage2
learn.unfreeze()
learn.fit_one_cycle(8, max_lr=slice(1e-6,3e-4))
learn.model_dir = Model_Path
learn.save('unfreezepretrain_alexnet_1')
...
print(data)
learn = cnn_learner(data, models.alexnet, pretrained=False, metrics=[error_rate, FBeta(average='weighted')], wd=1e-1, callback_fns=ShowGraph)
learn.model_dir = Model_Path
learn.load('unfreezepretrain_alexnet_1')
learn.fit_one_cycle(30, max_lr=5e-4)
learn.save('train_alexnet_1')
...
learn1 = cnn_learner(data_test, models.alexnet, pretrained=False, metrics=[error_rate, FBeta(average='weighted')], wd=1e-1, callback_fns=ShowGraph)
learn1.model_dir = Model_Path
learn1.load('train_alexnet_1')
